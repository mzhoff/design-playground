...
## Этап 3: сырой импорт UI-reference набора

Статус: выполнено в ветке `stage-3-raw-vertical-imports`.

### Что добавлено

- `packages/patterns/src/security-sphere-reference/types.ts` — временные контракты reference-набора.
- `packages/patterns/src/security-sphere-reference/fixtures.ts` — mock-данные для Storybook.
- `packages/patterns/src/security-sphere-reference/security-sphere-reference.tsx` — очищенные website/admin/forms previews.
- `packages/patterns/src/security-sphere-reference/README.md` — происхождение, ограничения и что не переносим.
- `apps/storybook/src/stories/security-sphere-reference.stories.tsx` — Storybook-сценарии.

### Решение по импорту

Перенос сделан как reference kit, а не как финальные компоненты. Из исходников Security Sphere убраны Next `Link/Image`, TanStack Query, app routing, analytics attributes, table libraries и доменная модель курсов/документов. Ценность набора — визуально сравнить ранний baseline с Gigonom 2026 и будущими Figma-слепками.

### Перенесенные паттерны

- Sticky website header behavior reference.
- Website catalog/content cards.
- Admin ListView/DataTable baseline.
- Admin CreateView/PageTabs/header actions shell.
- FileUploadField states.

### Известный технический долг

- После Figma-сравнения решить, что удалить как reference-only.
- Разнести полезные части по `website-blocks`, `admin-crud` и `forms`.
- Не переносить Security Sphere sidebar/menu до сравнения с Gigonom 2026 sidebar.
- Отдельно инвентаризировать Lexical editor stack, если он понадобится для editor-пакета.
