...
## Правило передачи этапа пользователю

После завершения этапа результат должен быть зафиксирован в git и отправлен в удаленный репозиторий.

Если локальные и удаленные проверки проходят, ассистент больше ничего не меняет в рамках этого этапа. Дальше пользователь сам проверяет результат и отдельно запускает следующую задачу или следующий этап.

Если проверки не проходят, ассистент продолжает только исправление текущего этапа. Новые задачи не смешиваются с исправлением упавших проверок.
